...
echo "🚀 Deploying Reactive DynamoDB Demo..."

# Validate region configuration
if [ -z "$AWS_REGION" ]; then
    echo "❌ AWS_REGION not set. Please configure your region."
    exit 1
fi

echo "📍 Deploying to region: $AWS_REGION"

# Check if CDK is bootstrapped for this region
ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
echo "🔍 Checking CDK bootstrap status for $AWS_REGION..."

if ! aws cloudformation describe-stacks --stack-name CDKToolkit --region $AWS_REGION >/dev/null 2>&1; then
    echo "⚠️  CDK not bootstrapped for region $AWS_REGION"
    echo "🔧 Bootstrapping CDK..."
    cdk bootstrap aws://$ACCOUNT_ID/$AWS_REGION
fi

# Build and push Spring Boot to ECR first
echo "🐳 Building and pushing Spring Boot to ECR..."

# Check if ECR repository exists and has images
REPO_EXISTS=$(aws ecr describe-repositories --repository-names $REPO_NAME --region $AWS_REGION 2>/dev/null || echo "false")
IMAGE_EXISTS="false"
...
if [ "$REPO_EXISTS" != "false" ]; then
    ...
    if [ "$IMAGE_COUNT" -gt "0" ]; then
        IMAGE_EXISTS="true"
        echo "📦 ECR repository $REPO_NAME already exists with $IMAGE_COUNT images"
    fi
fi

# Only deploy ECR stack if repository doesn't exist or has no images
if [ "$REPO_EXISTS" = "false" ] || [ "$IMAGE_EXISTS" = "false" ]; then
    ...
    cdk deploy ECRStack --app "$CDK_APP" --require-approval never --outputs-file ecr-outputs.json
    cd ..
else
    echo "⏭️  Skipping ECR Stack deployment - repository exists with images"
    # Create empty outputs file for consistency
    echo '{}' > infra/ecr-outputs.json
fi

# Create ECR repository if it doesn't exist (fallback)
...
# Tag and push
docker buildx build --platform linux/amd64 --push -t $ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/$REPO_NAME:latest .

echo "✅ Image pushed to: $ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/$REPO_NAME:latest"
...
# Deploy stacks one by one with proper dependencies
echo "📦 Deploying Frontend Stack..."
cdk deploy FrontendStack --app "$CDK_APP" --require-approval never --outputs-file frontend-outputs.json

echo "📦 Deploying Database Stack..."
cdk deploy DatabaseStack --app "$CDK_APP" --require-approval never --outputs-file database-outputs.json

echo "📦 Deploying Auth Stack..."
TABLE_ARN=$(cat database-outputs.json | grep -o '"TableArn": "[^"]*"' | cut -d'"' -f4)
cdk deploy AuthStack --app "$CDK_APP" --require-approval never --outputs-file auth-outputs.json --parameters TableArn="$TABLE_ARN"

echo "📦 Deploying VPC Stack..."
cdk deploy VPCStack --app "$CDK_APP" --require-approval never --outputs-file vpc-outputs.json

echo "📦 Deploying Messaging Stack..."
cdk deploy MessagingStack --app "$CDK_APP" --require-approval never --outputs-file messaging-outputs.json

# Extract outputs from individual files first
echo "📋 Extracting deployment outputs..."

BUCKET_NAME=$(cat frontend-outputs.json | grep -o '"BucketName": "[^"]*"' | cut -d'"' -f4)
echo "📝 Bucket Name: $BUCKET_NAME"

DISTRIBUTION_ID=$(cat frontend-outputs.json | grep -o '"DistributionId": "[^"]*"' | cut -d'"' -f4)
echo "📝 Distribution ID: $DISTRIBUTION_ID"

DOMAIN_NAME=$(cat frontend-outputs.json | grep -o '"DistributionDomainName": "[^"]*"' | cut -d'"' -f4)
echo "📝 Domain Name: $DOMAIN_NAME"

IDENTITY_POOL_ID=$(cat auth-outputs.json | grep -o '"IdentityPoolId": "[^"]*"' | cut -d'"' -f4)
...
echo "📝 MQ Broker ID: $MQ_BROKER_ID"

MQ_BROKER_ENDPOINT=$(cat messaging-outputs.json | grep -o '"MqBrokerEndpoint": "[^"]*"' | cut -d'"' -f4)
echo "📝 MQ Broker Endpoint: $MQ_BROKER_ENDPOINT"



# Validate all required outputs are present
if [ -z "$BUCKET_NAME" ] || [ -z "$DISTRIBUTION_ID" ] || [ -z "$DOMAIN_NAME" ] || [ -z "$IDENTITY_POOL_ID" ] || [ -z "$TABLE_NAME" ] || [ -z "$MQ_BROKER_ID" ] || [ -z "$MQ_BROKER_ENDPOINT" ]; then
    echo "❌ Missing required outputs from stack deployments"
    echo "Debug: Check output files exist and contain expected keys"
    ls -la *-outputs.json
    exit 1
fi

echo "📦 Deploying App Runner Stack..."
cdk deploy AppRunnerStack --app "$CDK_APP" --require-approval never --outputs-file apprunner-outputs.json

SERVICE_URL=$(cat apprunner-outputs.json | grep -o '"ServiceUrl": "[^"]*"' | cut -d'"' -f4)

echo "📦 Deploying Simple WebSocket Stack..."
cdk deploy SimpleWebSocketStack --app "$CDK_APP" --require-approval never --outputs-file simple-websocket-outputs.json

WEBSOCKET_URL=$(cat simple-websocket-outputs.json | grep -o '"SimpleWebSocketURL": "[^"]*"' | cut -d'"' -f4)
echo "📝 WebSocket URL: $WEBSOCKET_URL"

echo "📊 Deploying Monitoring Stack..."
cdk deploy MonitoringStack --app "$CDK_APP" --require-approval never --outputs-file monitoring-outputs.json

DASHBOARD_URL=$(cat monitoring-outputs.json | grep -o '"DashboardUrl": "[^"]*"' | cut -d'"' -f4)
echo "📝 Dashboard URL: $DASHBOARD_URL"

# Combine all outputs into apprunner-outputs.json for upload script
echo '{' > apprunner-outputs.json
cat frontend-outputs.json database-outputs.json auth-outputs.json vpc-outputs.json ecr-outputs.json messaging-outputs.json simple-websocket-outputs.json monitoring-outputs.json | grep '"[^"]*":' | sed 's#^[[:space:]]*##' | sed 's#$#,#' >> apprunner-outputs.json
echo '  "ServiceUrl": "'$SERVICE_URL'",' >> apprunner-outputs.json
echo '  "SimpleWebSocketURL": "'$WEBSOCKET_URL'",' >> apprunner-outputs.json
echo '  "DashboardUrl": "'$DASHBOARD_URL'"' >> apprunner-outputs.json
echo '}' >> apprunner-outputs.json

# Also create legacy outputs.json for compatibility
cp apprunner-outputs.json outputs.json
...
# Generate dynamic App.js with deployment values
echo "⚙️  Generating dynamic App.js..."
cat > src/App.js << EOF
...
const ALLOWED_API_BASE = '$SERVICE_URL';
const ALLOWED_WS_BASE = '$WEBSOCKET_URL';
